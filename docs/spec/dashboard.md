...
- **2 モデル比較**: 任意の 2 行をクリックで選択するとカード型の比較表示が出る（左右カード・共通スケールバー・トークン内訳ラベル・倍率要約 `unit price 6.1× · cost 2853× · tokens 470×`）。大きい側は赤（up）、小さい側は緑（down）で色分けする。3 つ目を選択すると古い方を置き換える。1 モデル選択時・未選択時は案内文言を表示する
- **状態と導線**: 選択期間はフィルタ・言語の変更後も維持される。「閉じる」ボタンまたは「全期間表示」でパネルを閉じる。パネル表示中も概要グラフのモデル集合・系列順・凡例は変わらない
