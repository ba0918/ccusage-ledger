...
- ccusage コマンドの実行は safe-chain（パッケージ整合性を検査するユーティリティ）を介する。safe-chain はシェルの関数ラッパーとして動作するため、アプリ内部からの子プロセス起動では素通りする。そのため spawn を safe-chain 経由にし、safe-chain が無い環境では直接 bunx にフォールバックする
